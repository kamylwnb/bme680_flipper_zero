...
static bool init_bme680(BME680App* app) {
    ...
    if(!app->gas_stabilization_period) {
        ...
        FURI_LOG_I(
            "BME680", "Sensor initialized successfully. Gas stabilization period started (30s).");
    } else {
        FURI_LOG_I(
            "BME680", "Sensor initialized successfully. Gas stabilization already in progress.");
    }
    ...
    if((rslt == BME68X_OK) && (n_fields > 0)) {
        ...
        if(app->gas_enabled && app->gas_resistance > 0) {
            ...
            if(app->gas_stabilization_period) {
                ...
                new_safety_state =
                    evaluate_gas_safety(app->gas_resistance, app->gas_alarm_threshold_kohm);
            }
                ...
                if(app->gas_enabled && app->gas_safety_state != GasSafety_Unknown) {
                    ...
                    if(app->gas_stabilization_period) {
                        ...
                        uint32_t elapsed_seconds =
                            current_time - app->gas_stabilization_start_time;
                        uint32_t remaining_seconds = GAS_STABILIZATION_TIME_SEC - elapsed_seconds;

                        if(remaining_seconds > GAS_STABILIZATION_TIME_SEC)
                            remaining_seconds = 0; // Handle overflow
                        ...
                        case GasSafety_Safe:
                            snprintf(
                                buf, sizeof(buf), "✓ SAFE (>%lukΩ)", app->gas_alarm_threshold_kohm);
                            break;
                        case GasSafety_Dangerous:
                            snprintf(
                                buf,
...
static void draw_start_confirm_screen(Canvas* canvas, BME680App* app) {
    ...
    if(cooldown_remaining > 0) {
        ...
        canvas_draw_str_aligned(
            canvas, 64, 55, AlignCenter, AlignTop, "[OK] Start  [Back] Cancel");
    }
}
                ...
                if(app->settings_cursor == SettingsItem_Address) {
                    ...
                    if(app->is_sensor_initialized) {
                        ...
                        if(app->gas_enabled) {
                            ...
                            app->gas_stabilization_period = true;
                            FURI_LOG_I(
                                "BME680", "Gas sensor enabled. Stabilization period started.");
                        }
                    }
                    config_changed = true;
